...
getContextDiffNew context a b =
    groupBy' (\a' b' -> not (isThese a' && isThese b')) $ doPrefix $ getGroupedDiff a b
    where
      isThese (These {}) = True
      isThese _          = False
      ...
      doSuffix [] = []
      doSuffix [These xs ys] = [These (maybe xs (`take` xs) context) (maybe ys (`take` ys) context)]
      doSuffix (These xs ys : more)
          | maybe True (\n -> length xs <= n * 2) context =
              These xs ys : doPrefix more
      doSuffix (These xs ys : more) =
          These (maybe xs (`take` xs) context) (maybe ys (`take` ys) context)
                   : doPrefix (These (maybe mempty (`drop` xs) context) (maybe mempty (`drop` ys) context) : more)
...
getContextDiffOld context a b =
    ...
    where
      -- Drop the middle elements of a run of These if there are more
      -- than enough to form the context of the preceding changes and
      -- the following changes.
      split (These xs ys) =
          case length xs of
            n | n > 2 * context -> [These (take context xs) (take context ys), These (drop (n - context) xs) (drop (n - context) ys)]
            _ -> [These xs ys]
      split x = [x]
      -- If split created a pair of These runs at the beginning or end
      -- of the diff, remove the outermost.
      trimHead []                                 = []
      trimHead [These _ _]                        = []
      trimHead [These _ _, These _ _]             = []
      trimHead (These _ _ : x@(These _ _) : more) = x : more
      trimHead xs                                 = trimTail xs
      trimTail [x@(These _ _), These _ _] = [x]
      trimTail (x : more)                 = x : trimTail more
      trimTail []                         = []
      -- If we see That before This swap them so that the deletions
      -- appear before the additions.
      swap (x@(That _) : y@(This _) : xs) = y : x : swap xs
      swap (x : xs)                       = x : swap xs
      swap []                             = []
      -- Split the list wherever we see adjacent These constructors
      group xs =
          groupBy (\ x y -> not (isThese x && isThese y)) xs
          where
            isThese (These _ _) = True
            isThese _           = False
...
prettyContextDiff old new prettyElem hunks =
    ...
    where
      ...
      prettyChange (These ts _) = map (\ l -> text " " `mappend` prettyElem l) ts
      prettyChange (This ts)  = map (\ l -> text "-" `mappend` prettyElem l) ts
      prettyChange (That ts) = map (\ l -> text "+" `mappend` prettyElem l) ts
